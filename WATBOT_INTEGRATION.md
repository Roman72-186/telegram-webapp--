# Интеграция Mini App с WatBot

Инструкция получению Telegram ID пользователя и запуску сценариев в WatBot.
...
## 2. Как ID отправляется в WatBot

При оформлении заказа Mini App отправляет POST-запрос на вебхук:

```javascript
// app.js - функция sendToWatBot()
...
## 3. Настройка WatBot для приёма заказов

### Шаг 1: Создайте бота для регистрации контактов

Пользователь **должен сначала написать боту**, чтобы WatBot создал контакт с его `telegram_id`.

```
Пользователь → Пишет /start боту → WatBot создаёт контакт с telegram_id
```

### Шаг 2: Настройте Webhook в WatBot

1. Перейдите в **WatBot → Настройки → Webhooks**
2. Создайте **Webhook**
3. Скопируйте URL: `https://api.watbot.ru/hook/4325587:oAbD2q949K3SGmOKvZXF2gjtO4d1LHrTnHNWX4T2h8dB6DO8`

### Шаг 3: Правильное подключение MAX Bridge SDK

Для корректной работы приложения в мессенджере Max, используйте следующий URL для подключения SDK:

```html
<script src="https://st.max.ru/js/max-web-app.js"></script>
```

Это критически важно, так как использование неправильного URL может привести к тому, что методы `WebApp` не будут найдены.

### Шаг 3: Создайте сценарий обработки заказа

В WatBot создайте сценарий, который:

```
Триггер: Webhook (ваш webhook)
    ↓
...
│     - Перенаправляет на WatBot webhook                         │
└─────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────┐
...
**Контакт должен существовать в WatBot до оформления заказа!**

Пользователь должен сначала:
1. Написать `/start` вашему Telegram боту
2. WatBot создаст контакт с его `telegram_id`
...
| `js/app.js` | Логика оформления заказа и отправки в WatBot |
| `js/config.js` | Конфигурация (WEBHOOK_URL, данные товара) |
| `index.html` | Основной файл приложения с правильным подключением MAX Bridge SDK |
...
✅ Заказ успешно отправлен в WatBot
```
